...
public enum MimeExtensionMapper {
	...
	private MimeExtensionMapper() {
		try {
			InputStream rawMappings = MimeExtensionMapper.class.getClassLoader()
                .getResourceAsStream(mappingResourcePath);
			processMappings(rawMappings);
		} catch (IOException excep) {
			excep.printStackTrace();
			throw new IllegalStateException("Problem reading internal mapping file", excep);
		}
	}
	...
	private static void processMappings(final InputStream rawMappings) throws IOException {
	...
	private static void processMapping(final String line) {
		...
		if (mimeExtMap == null) System.out.println("NULL MAP");
		if (parts.length >= 2) {
